...
const LunchPlanningScreen = () => {
  ...
  const handleContinue = () => {
    ...
    console.log('Lunch planning data:', timeData);
    alert(`Lunch planned for ${timeData.time} within ${radius} miles. Import settings: ${importFromBefore ? 'Yes' : 'No'}`);
    setOnboardingData({
      ...onboardingData,
      lunchTime: timeData,
      step: 'dinner.js',
        ...
        <div className="absolute bottom-20 left-6 right-6">
          <div className="max-w-md mx-auto">
            <div className={`text-center text-sm ${colors.textSecondary}`}>
              {radius <= 2 && "Perfect for nearby lunch spots within walking distance"}
              {radius > 2 && radius <= 10 && "Great range for local lunch favorites"}
              {radius > 10 && "Exploring lunch options across the wider area"}
            </div>
          </div>
        </div>
      )}
...
export default LunchPlanningScreen;